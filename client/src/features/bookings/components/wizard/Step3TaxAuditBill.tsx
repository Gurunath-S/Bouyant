...
export const Step3TaxAuditBill: React.FC<Step3TaxAuditBillProps> = ({
  ...
  if (!selectedCompany) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center space-y-4 max-w-xl mx-auto shadow-xs">
        <AlertTriangle className="w-10 h-10 text-amber-500 mx-auto" />
        <h3 className="text-lg font-bold text-[#012970]">Corporate Details Required</h3>
        <p className="text-xs text-slate-500">Please provide and confirm your company details to generate the Tax Invoice & Bill.</p>
  ...
  if (selectedStalls.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center space-y-4 max-w-xl mx-auto shadow-xs">
        <AlertTriangle className="w-10 h-10 text-amber-500 mx-auto" />
        <h3 className="text-lg font-bold text-[#012970]">No Stalls Selected</h3>
        <p className="text-xs text-slate-500">Please select at least one available stall from the floor plan to view the Tax Bill.</p>
        <Button variant="primary" onClick={onBack} className="bg-[#09539b]">
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
            ...
            <div className="p-4 divide-y divide-slate-100 bg-white">
              {selectedStalls.map((stall) => (
                <div key={stall.id} className="py-3 flex items-center justify-between text-xs">
                  <div>
                    <span className="font-bold text-[#012970] font-mono mr-2">#{stall.stallNumber}</span>
                    <span className="text-slate-500 uppercase">{stall.category}</span>
                    <span className="text-slate-400 ml-2">({stall.areaSqFt} Sq.Ft)</span>
                  </div>
                  <span className="font-mono font-bold text-[#09539b]">₹{Number(stall.price).toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-5 bg-white border-2 border-[#09539b]/30 rounded-xl space-y-4 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 mt-1">Pay complete booth rental today with zero pending balance.</p>
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 mt-1">Pay advance today to lock booth; balance due 15 days before event.</p>
                <div className="mt-3 pt-2 border-t border-slate-100 flex justify-between items-center text-xs font-bold text-[#012970]">
            ...
            {paymentType === 'PARTIAL' && (
              <div className="p-4 bg-amber-50/80 border border-amber-200 rounded-xl space-y-3 animate-in fade-in">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <label className="text-xs font-bold text-slate-700">Select Advance Percentage (Must be &gt; 50%):</label>
                ...
                <div className="flex items-center gap-3 bg-white p-3 rounded-lg border border-amber-200">
                  ...
                  <span className="text-xs text-slate-500 font-medium">
                    (Payable Today: <b className="font-mono text-[#09539b]">₹{payableToday.toLocaleString()}</b> • Balance:{' '}
                    <b className="font-mono text-slate-800">₹{remainingBalance.toLocaleString()}</b>)
                  </span>
                </div>
          ...
          <div className="p-5 bg-slate-50 border-2 border-slate-200 rounded-xl space-y-4 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-2">
              <h4 className="font-extrabold text-[#012970] text-sm flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-[#09539b]" /> Official Exhibitor Agreement
              </h4>
              <Button
                type="button"
                variant="outline"
            ...
            <div className="flex items-start gap-3 pt-2">
              ...
              />
              <label htmlFor="termsAccepted" className="text-xs text-slate-700 font-medium">
                I have reviewed the booking details and accept the{' '}
                <button
                  type="button"
                  onClick={() => setIsTermsModalOpen(true)}
                  className="text-[#09539b] font-bold underline hover:text-[#012970]"
                >
        ...
        <div className="space-y-4">
          <div className="bg-[#012970] text-white p-5 rounded-2xl shadow-lg space-y-4 sticky top-4">
