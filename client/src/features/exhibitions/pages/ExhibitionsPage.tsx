...
import { Calendar, MapPin, Search, Layers, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white border border-slate-200 rounded-xl">
          <p className="text-slate-600 text-sm font-semibold">No exhibitions match your search.</p>
        </div>
      ) : (
          ...
          {filtered.map((expo) => (
            ...
            >
              <div className="h-40 bg-slate-800 relative overflow-hidden">
                ...
                />
                <span className="absolute top-3 right-3 px-2.5 py-1 bg-white/90 backdrop-blur-xs text-slate-900 font-extrabold text-[10px] rounded uppercase shadow-2xs">
                  {expo.status}
                </span>
              </div>

              <div className="p-5 space-y-3">
                <h3 className="text-base font-bold text-slate-900 leading-snug">{expo.title}</h3>
                <p className="text-xs text-slate-600 line-clamp-2">{expo.description}</p>
