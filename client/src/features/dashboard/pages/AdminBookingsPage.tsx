    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs space-y-3">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-purple-500/30 font-medium"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            <div className="flex items-center gap-1.5">
              ...
              <select
                ...
                className="text-xs p-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-semibold text-slate-700"
              >
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-100 font-medium">
          ...
          <span>
            Total Filtered Value: <strong className="font-mono text-purple-700">₹{totalValue.toLocaleString()} INR</strong>
          </span>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-800">
              {filteredBookings.map((b) => (
                ...
                >
                  <td className="py-3.5 px-4 font-mono font-bold text-purple-700 group-hover:underline">
                    {b.bookingReference}
                  </td>
                  <td className="py-3.5 px-4">
                    <span className="font-bold text-slate-900 block truncate max-w-xs">{b.exhibition?.title}</span>
                  ...
                  <td className="py-3.5 px-4">
                    <span className="font-bold block text-slate-900">{b.company?.name}</span>
                    <span className="text-[10px] text-slate-400 block">{b.company?.contactPerson}</span>
                  </td>
                  <td className="py-3.5 px-4">
