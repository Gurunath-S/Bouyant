...
export const StallFilterBar: React.FC<StallFilterBarProps> = ({ onZoomChange, currentZoom }) => {
  const { selectedCategory, setSelectedCategory, selectedHall, setSelectedHall } = useFloorPlanStore();
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-xs space-y-3">
      {/* Top Row: Pavilion / Hall Tabs & Tariff Badges */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-2.5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-black text-[#012970] dark:text-slate-200 uppercase tracking-wider">
            Hall Pavilion:
          </span>
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            >
              All Pavilions (Hall A & B)
            </button>
            <button
              onClick={() => setSelectedHall('HALL_A')}
              className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${
                selectedHall === 'HALL_A'
            ...
            <button
              onClick={() => setSelectedHall('HALL_B')}
              className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${
                selectedHall === 'HALL_B'
                  ? 'bg-[#09539b] text-white shadow-2xs'
                  : 'text-slate-600 dark:text-slate-400 hover:text-slate-900'
        ...
        <div className="flex items-center gap-3 text-[11px] font-bold">
          <span className="px-2.5 py-1 bg-[#EEF4FC] text-[#09539b] rounded-lg border border-[#09539b]/20">
            Regular Stall: <b className="font-mono">₹6,500 / Sqm</b>
          </span>
          <span className="px-2.5 py-1 bg-amber-50 text-amber-900 rounded-lg border border-amber-300">
            Corner / Island: <b className="font-mono text-amber-950">₹7,000 / Sqm</b>
          </span>
