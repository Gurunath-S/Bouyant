...
import { StallCategory, StallStatus, Stall } from '../../../types';
...
export const StallFilterBar: React.FC<StallFilterBarProps> = ({ stalls, onZoomChange, currentZoom }) => {
  const { selectedCategory, setSelectedCategory, selectedStatus, setSelectedStatus } = useFloorPlanStore();
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3.5 shadow-xs flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-4 text-xs font-medium text-slate-600">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-amber-100 border border-amber-500 inline-block" />
          Held
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-slate-200 border border-slate-400 inline-block" />
          Booked
        </span>
        <span className="flex items-center gap-1.5">
      ...
      <div className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 p-1 rounded-lg">
        ...
        <span className="text-xs font-mono font-bold text-slate-800 px-2">{currentZoom}%</span>
        <button
        ...
        <button
          ...
          className="p-1 rounded text-slate-600 hover:bg-white transition-colors ml-1"
          title="Reset Zoom"
        >
